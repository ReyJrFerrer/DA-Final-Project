...
def evaluate_model(model, X, y, model_name, dataset_name):
    ...
    plt.savefig(f'results/svm/{model_name}_{dataset_name}_confusion_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.savefig(f'results/svm/{model_name}_{dataset_name}_roc_curve.png', dpi=300, bbox_inches='tight')
    plt.close()
...
results_df.to_csv(f'results/svm/svm_results.csv', index=False)
